...
impl Mount {
    ...
    pub async fn init(ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<Self, MountError> {
    ...
    pub async fn pull(cid: Cid, ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<Self, MountError> {
    ...
    pub async fn hash_data<R>(data: R, ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<Cid, MountError>
    where
        R: Read + Send + Sync + 'static + Unpin,
    {
    ...
    pub async fn add_data<R>(data: R, ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<Cid, MountError>
    where
        R: Read + Send + Sync + 'static + Unpin,
    {
    ...
    async fn cat_data(cid: &Cid, ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<Vec<u8>, MountError> {
        let data = ipfs_rpc.cat_data(cid).await?;
        Ok(data)
    }

    async fn get<B>(cid: &Cid, ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<B, MountError>
    where
        B: TryFrom<Ipld>,
    {
    ...
    async fn put<B>(object: &B, ipfs_rpc: &IpfsRpc<IpfsClient>) -> Result<Cid, MountError>
    where
        B: Into<Ipld> + Clone,
    {
